  ...
  const contacts = [
    {
      ...
      label: t("contactSection.addressLabel"),
    ...
    },
    {
      ...
      label: t("contactSection.hoursLabel"),
      value: t("contactSection.hours"),
    },
  ];

  return (
    <section ref={ref} className="w-full bg-white py-12 sm:py-16 md:py-20 px-4">
      <div className="max-w-6xl mx-auto">
        ...
        >
          ...
          <h2 className="text-[#1A3FA0] text-2xl sm:text-3xl md:text-4xl font-extrabold mt-2">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 sm:gap-8 items-stretch">
          ...
          >
            {contacts.map(({ icon, label, value, href }, i) => (
              <motion.div
                ...
                className="flex items-start gap-3 sm:gap-4 p-4 sm:p-5 rounded-2xl border border-gray-100
                           hover:border-gold/30 hover:shadow-md transition-all duration-300 group"
              >
                <div className="w-10 h-10 rounded-xl bg-[#1A3FA0]/5 flex items-center justify-center
                                group-hover:bg-[#1A3FA0]/10 transition-colors duration-300 shrink-0">
                  {icon}
                </div>
                <div>
                  <p className="text-[11px] sm:text-xs font-bold uppercase tracking-widest text-gray-400 mb-0.5">
                    {label}
                  </p>
                  {href ? (
                    <a href={href} className="text-sm sm:text-base text-gray-800 font-medium hover:text-[#1A3FA0] transition-colors duration-200">
                      {value}
                    </a>
                  ) : (
                    <p className="text-sm sm:text-base text-gray-800 font-medium">{value}</p>
                  )}
                </div>
              </motion.div>
            ))}
          ...
          <motion.div
            ...
            className="rounded-2xl overflow-hidden shadow-xl border border-gray-100 min-h-[280px] sm:min-h-[360px]"
          >
            <iframe
