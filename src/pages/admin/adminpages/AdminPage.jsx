  ...
  >
    <Link to={to} className="block group">
      <div
        ...
        style={{
          background: "#fff",
          border: "2px solid #1A3FA00D",
          boxShadow: "0 2px 16px rgba(26,63,160,0.07)",
        }}
        onMouseEnter={(e) => {
        ...
        onMouseLeave={(e) => {
          e.currentTarget.style.borderColor = "#1A3FA00D";
          e.currentTarget.style.boxShadow = "0 2px 16px rgba(26,63,160,0.07)";
        }}
      >
        ...
        <p className="text-3xl font-bold mb-1" style={{ color: "#1A3FA0" }}>{value}</p>
        <p className="text-sm font-medium" style={{ color: "#1A3FA077" }}>{label}</p>
      </div>
    </Link>
  </motion.div>
);
...
const QuickAction = ({ icon, label, desc, to, delay }) => (
  ...
  >
    <Link to={to} className="flex items-center gap-4 p-4 rounded-xl group transition-all duration-200 hover:bg-[#1A3FA008]">
      <div
      ...
      <div className="flex-1">
        <p className="font-bold text-sm" style={{ color: "#1A3FA0" }}>{label}</p>
        <p className="text-xs" style={{ color: "#1A3FA066" }}>{desc}</p>
      </div>
...
export default function AdminPage() {
  ...
  return (
    <div className="space-y-8 max-w-6xl">
      ...
      {/* Статистика */}
      <div>
        <h3 className="text-sm font-bold uppercase tracking-widest mb-4" style={{ color: "#1A3FA077" }}>
          Обзор
        </h3>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <motion.div
          ...
          style={{ background: "#fff", border: "2px solid #1A3FA00D", boxShadow: "0 2px 16px rgba(26,63,160,0.07)" }}
        >
          <h3 className="font-bold text-base mb-4" style={{ color: "#1A3FA0" }}>Быстрые действия</h3>
          <div className="divide-y" style={{ borderColor: "#1A3FA008" }}>
        ...
        >
          <div className="flex items-center justify-between mb-4">
            <h3 className="font-bold text-base" style={{ color: "#1A3FA0" }}>Последние новости</h3>
            <Link to="/admin/news" className="text-xs font-bold" style={{ color: "#C4973A" }}>Все →</Link>
          </div>
          <div className="space-y-3">
            {news.slice(0, 4).map((item) => (
              <div key={item.id} className="flex items-center gap-3 p-2 rounded-xl hover:bg-[#1A3FA005] transition-colors">
                <img
                  src={item.image}
                  alt={item.title}
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-semibold truncate" style={{ color: "#1A3FA0" }}>{item.title}</p>
                  <p className="text-xs" style={{ color: "#1A3FA066" }}>{item.date}</p>
                </div>
                <span
                  className="text-xs font-bold px-2 py-0.5 rounded-full flex-shrink-0"
                  style={{ background: "#1A3FA00D", color: "#1A3FA099" }}
                >
                  {item.category}
                </span>
              </div>
